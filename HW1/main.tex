...
Similar to Bragg reflection and the two slit experiment, these interference effects can be constructive and destructive (or some intermediate between the two).
...
Actually, this estimation may not be quite correct.
...
More rigorously, we should write $E = \gamma Mc^2 = \sqrt{1 + (p/Mc)^2} Mc^2$, which means that the relation between a relativistic particle and its energy is given by
\begin{eqnarray}
...
Let $\va*{r}_{0}$ be a position at time $t = 0$ where $\Psi = \Psi(\va*{r}_{0},0)$ then at time $t$, $\Psi(\va*{r}_{0} + t \grad_{\va*{k}} \omega,t) \approx \exp(-i [ \omega - \va*{k} \cdot \grad_{\va*{k}} \omega ]) \Psi(\va*{r}_{0},0)$.
Essentially, the wavefunction changes in time by translating in space an amount $t \grad_{\va*{k}} \omega$ and by a phase, although this has a norm of unity and therefore does not have any impact on the physical interpretation.
That is, $| \Psi(\va*{r}_{0} - t \grad_{\va*{k}} \omega,0) | \approx | \Psi(\va*{r}_{0},0) |$.
