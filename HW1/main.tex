...
.\end{eqnarray}
olving for $\dd{r}/\dd{t}$ gives a differential equation for $r$ as
\begin{eqnarray}
...
path length $R_1 - R_0 = n \lambda$, where $\lambda$ is the wavelength of the neutron and $n \in \mathbb{Z}$, then we expect the neutron waves to be in phase and to therefore interfere constructively, and if $R_1 - R_0 = (n + 1/2) \lambda$, then the waves destructively interfere.
Since $\lambda \propto 1/\sqrt{E}$, these intensity maxima and nodes become more closely packed since $R_1 - R_2 \approx \frac{n}{\sqrt{E}}$ for the maxima (and similarly for the nodes)\footnote{Actually, this rationale gives us the more ``classical'' expectation for small $E$ since if $E \rightarrow 0$, then $R_1 - R_0 \rightarrow \infty$, implying that the intensity changes slowly as a function of $\theta$.}.

(b) Let's make the previous discussion more quantitative.
If we assume that the path along $R_1$ is approximately parallel to $R_0$\footnote{This is approximately correct since $d \ll R_0,R_1$} then we can write $R_1 - R_0 \approx d \sin{\theta} = n \lambda = n h/\sqrt{2ME}$.
...
(c) The general guidance for when the target size must be considered is when the wavelength of the incoming particles is on the same scale as the size of the target particles, which in this case is about $1~{\rm fm}$ or $10^{-13}~{\rm cm}$.
...
In this high energy regime, we should write $E = \gamma Mc^2 = \sqrt{1 + (p/Mc)^2} Mc^2$, which means that the relation between a relativistic particle and its energy is given by
\begin{eqnarray}
