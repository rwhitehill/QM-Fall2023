...
By taking $Z=1$ and $R \approx \SI{e-8}{\centi\metre}$ appropriate for the hydrogen atom, justify this assumption \textit{a posteriori} by comparing $\tau$ with the revolution period.
...
Since the acceleration is instantaneously centripetal\footnote{This is approximately true if we assume that $\tau$ is much larger than the revolution period. Under this assumption, the energy lost per revolution is much smaller than the energy of the system, and thus, that the electron's tangential component of acceleration is very small.} we can write
\begin{eqnarray}
...
.\end{eqnarray}
solving for $\dd{r}/\dd{t}$ gives a differential equation for $r$ as
\begin{eqnarray}
    \dv{r}{t} = -\frac{4 e^2}{3c^3} \frac{1}{m^2} \frac{Z e ^2}{r^2}
.\end{eqnarray}
We can solve this as follows
\begin{eqnarray}
    \int_{R}^{0} r^{2} \dd{r} = -\frac{R^3}{3} = -\frac{4 Z e^{4}}{3 m^2 c^3} \tau
.\end{eqnarray}
And finally we rearrange to find 
\begin{eqnarray}
    \eqbox{ \tau = \frac{m^2 c^3}{Z e^{4}} \frac{R^3}{4} \approx \frac{1}{Z} \times 10^{-10}~{\rm s}}
.\end{eqnarray}
...
,\end{eqnarray}
which is quite tiny for any relevant $Z$ (including $Z = 1$ for the hydrogen atom) and at least tells us that our results are consistent with our assumptions.

{
\color{red}

The statement that the acceleration is instantaneously centripetal is always true: the force is always central.
It is that the motion is instaneously circular that allows us to write $F = mv^2/r$
...
The equipartition theorem states that for a system of $N$ particles with 2 quadratic degrees of freedom per particle
\begin{eqnarray}
...
\begin{eqnarray}
    \expval{E} = -\frac{1}{\cal Z} \pdv{\cal Z}{\beta} = -\pdv{\ln{\cal Z}}{\beta}
,\end{eqnarray}
...
Now, we can derive the heat capacity of a solid by setting the energy of the solid to be $E = 3 N \expval{E_o}$.
Then
\begin{eqnarray}
    \begin{aligned}
...
\textit{ In retrospect, since we were only concerned here with the behavior of $c_{V}$ as $T \rightarrow 0,\infty$, it is likely simpler to define $\tilde{E} = E/(9 N h \nu_{\rm max})$, consider the limiting behavior of $\tilde{E}(T)$ at leading order in $T$ (or $1/T$), and let $\tilde{c}_{V} = (\partial_{T} \tilde{E})_{V}$.
...
(a) At very low energy, the wavelength of the nuetrons would be very large, and hence would be almost like classical particles, meaning that the intensity distribution for small $E$ would be quite flat.
At higher energy scales, the neutrons wavelength would become comparable to the scale $d$ of the separation distance between the atomic nuclei.
Hence at this energy scale, the wave-like nature of the neutrons becomes important, as the neutrons interfere with each other creating a more rich intensity distribution from scattering off the linear chain of atomic nuclei.
Similar to Bragg reflection and the two slit experiment, these interference effects can be constructive or destructive (or some intermediate between the two).
If the path length $|R_1 - R_0| = n \lambda$, where $\lambda$ is the wavelength of the neutron and $n \in \mathbb{Z}$, then we expect the neutron waves to be in phase and to therefore interfere constructively, and if $|R_1 - R_0| = (n + 1/2) \lambda$, then the waves destructively interfere.
Since $\lambda \propto 1/\sqrt{E}$, these intensity maxima and nodes become more closely packed since $|R_1 - R_0| \approx \frac{n}{\sqrt{E}}$ for the maxima (and similarly for the nodes)\footnote{Actually, this rationale gives us the more ``classical'' expectation for small $E$ since if $E \rightarrow 0$, then $|R_1 - R_0| \rightarrow \infty$, implying that the intensity changes slowly as a function of $\theta$.}.

(b) Let's make the previous discussion more quantitative.
If we assume that the path along $R_1$ is approximately parallel to $R_0$\footnote{This is valid since we assume $d \ll R_0,R_1$} then we can write $|R_1 - R_0| \approx d \sin{\theta} = n \lambda = n h/\sqrt{2ME}$.
Thus, (taking n = 1) at $\theta = 30^{\circ}$ and $E_1 = 1.3 \times 10^{-13}~{\rm erg}$, we find
\begin{eqnarray}
    \eqbox{ d = \frac{2 h}{\sqrt{2 (1.67 \times 10^{-24}~{\rm g})(1.3 \times 10^{-13}~{\rm erg})}} \approx 2 \times 10^{-8}~{\rm cm} }
.\end{eqnarray}


(c) The ``rule of thumb'' for when the target size must be considered is that the wavelength of the incoming particles is on the same scale as the size of the target particles, which in this case is about $1~{\rm fm}$ or $10^{-13}~{\rm cm}$.
Doing the quick estimations, we find
\begin{eqnarray}
    \lambda = \frac{h}{\sqrt{2ME}} \approx R \Rightarrow E \approx \frac{h}{2MR^2} \approx 1.2 \times 10^{-3}~{\rm erg}~{\rm or}~0.7 \times 10^{9}~{\rm eV}
.\end{eqnarray}

This estimation may not be quite correct, though.
Observe that the classical speed of such a particle is $v = \sqrt{2E/m} \approx 1.25 c$, which is a clear violation of a fundamental postulate of relativity.
In this high energy regime, we should write $E = c \sqrt{(Mc)^2 + p^2}$, which means that the relation between a relativistic particle and its energy is given by
\begin{eqnarray}
    E = \sqrt{1 + \frac{1}{\lambda^2} \Big( \frac{h}{Mc} \Big)^2} Mc^2
.\end{eqnarray}
Taking $\lambda \approx R$, we have the energy at which the nuclear size becomes relevant to be $\eqbox{ E \approx 2.4 \times 10^{-3}~{\rm erg}~{\rm or}~1.5 \times 10^{9}~{\rm eV} }$, which is about double the prediction using the classical formulae.
...
\begin{eqnarray}
   \omega' \approx \omega +  (\va*{k}' - \va*{k}) \cdot \grad_{\va*{k}} \omega
.\end{eqnarray}
In this case, we can then write $\Psi$ as
\begin{eqnarray}
    \Psi(\va*{r},t) \approx \int \frac{\dd[3]{\va*{k}'}}{(2\pi)^{3/2}} f(\va*{k}') e^{i \big( \va*{k}' \cdot \va*{r} - \big[ \omega + (\va*{k}' - \va*{k}) \cdot \grad_{\va*{k}} \omega \big] t \big)}
.\end{eqnarray}
We can then take out any exponential factors which are independent of $\va*{k}'$, which gives
\begin{eqnarray}
    \label{eq:psi-approx}
    \eqbox{
    \begin{aligned}
        \Psi(\va*{r},t) &\approx e^{-i \big( \omega - \va*{k} \cdot \grad_{\va*{k}} \omega \big) t} \int \frac{\dd[3]{\va*{k}'}}{(2\pi)^{3/2}} f(\va*{k}') e^{i \va*{k}'  \cdot ( \va*{r} - t \grad_{\va*{k}} \omega )} \\
                        &= e^{-i \big( \omega - \va*{k} \cdot \grad_{\va*{k}} \omega \big) t} \Psi(\va*{r} - t \grad_{\va*{k}} \omega, 0)
    \end{aligned}
}
.\end{eqnarray}
Note that the factor $\grad_{\va*{k}} \omega = \grad_{\va*{k}'} \omega'|_{\va*{k}' = \va*{k}}$ is in fact independent of $\va*{k}'$ since we make the replacement $\va*{k}' = \va*{k}$ after taking the gradient with respect to $\va*{k}'$.

Consider $\va*{r} = \va*{r}_{0} + t \grad_{\va*{k}} \omega$ then at time $t$, $\Psi(\va*{r}, t) \approx \exp(-i [ \omega - \va*{k} \cdot \grad_{\va*{k}} \omega ]) \Psi(\va*{r}_{0},0)$.
Essentially, the wavefunction changes in time by translating in space an amount $t \grad_{\va*{k}} \omega$ and by a global phase, although this has a norm of unity and therefore does not manifest itself in a physically meaningful way.
That is, $| \Psi(\va*{r},t) | = | \Psi(\va*{r}- t \grad_{\va*{k}} \omega,0) | \approx | \Psi(\va*{r}_{0},0) |$.

(b) From part (a) we know that the wave packet travels at the group velocity
\begin{eqnarray}
