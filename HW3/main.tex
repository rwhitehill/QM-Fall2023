...
\begin{align}
    ...
    \label{eq:prob4-c}
    Ae^{\kappa a} - B e^{-\kappa a} + C e^{- \kappa a} &= - v_0 Ce^{-\kappa a}
,\end{align}
...
Thus, for a non-trivial solution $\{ A,B,C \} $, \erefs{prob4-a}{prob4-c} must be linearly dependent or
\begin{gather}
   \begin{vmatrix}
       1 & 1 & 0 \\
       e^{2 \kappa a} & 1 & -1 \\
       e^{2 \kappa a} & -1 & (1 + v_0)
   \end{vmatrix}
   = [(1 - v_0) - 1] - [e^{2 \kappa a}(1 + v_0) + e^{2 \kappa a}] = 0 \\
   e^{2 \kappa a} = \frac{v_0}{2 + v_0} \Rightarrow \kappa = \frac{1}{2a} \ln( \frac{v_0}{2 + v_0} )
.\end{gather}
Thus, there is one bound state permitted under this potential.
